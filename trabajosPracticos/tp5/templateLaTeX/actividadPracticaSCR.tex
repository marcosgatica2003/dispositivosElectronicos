...
\hline
\cellcolor[HTML]{FFFFC7}\textbf{$I_G$ $[mA]$} & \cellcolor[HTML]{FFFFC7}\textbf{$V_{CC}$ $[V]$} & \cellcolor[HTML]{FFFFC7}\textbf{$I_{AK}$ $[mA]$} & \cellcolor[HTML]{FFFFC7}\textbf{$V_{AK}$ $[V]$} \\ \hline
\textbf{1,47}                                 & 10,68                                           & 1,966                                            & 1,44                                            \\
\textbf{1,59}                                 & 11,46                                           & 2,1290                                           & 1,46                                            \\
\textbf{1,74}                                 & 12,35                                           & 2,3216                                           & 1,43                                            \\
\textbf{1,91}                                 & 13,23                                           & 2,5524                                           & 1,23                                            \\
\textbf{2,12}                                 & 14,43                                           & 2,8344                                           & 1,11                                            \\ \hline
\end{tabular}
% \begin{tabular}{|
% >{\columncolor[HTML]{ECF4FF}}c |c|c|c|}
% \hline
...
\sangria{} Con los datos obtenidos se puede observar que a mayor corriente de compuerta $I_G$, menor es la tensión $V_{AK}$ necesaria para que el SCR dispare. Esto se debe a que una mayor corriente en la compuerta facilita la conducción en la unión PN, haciendo que el dispositivo dispare antes.
\begin{itemize}[nosep]
    ...
    \item\textbf{Elaborar conclusiones del gráfico realizado.}
\end{itemize}
\sangria{} En el gráfico se puede observar que el SCR esta en región de bloque en directa hasta $V_{AK}=0,6V$ para todos los valores de $I_G$. A partir de ese punto, la corriente $I_{AK}$ se dispara indicando que el SCR comenzó a conducir. Se puede ver que a mayor corriente de compuerta $I_G$, hay una mayor pendiente y aumenta la corriente $I_{AK}$ para un mismo valor de $V_{AK}$. Esto demuestra que a más corriente de compuerta, mayor es la corriente entre anodo y cátodo.  
...
\end{itemize}
\sangria{} El ángulo mínimo de disparo se da para $R_2 = 0 \Omega$, en este caso el capacitor se carga instantáneamente y el SCR dispara casi al comienzo del semiciclo $\simeq 0^o$. El ángulo máximo se da para $R_2 =4K9 \Omega$, en este caso el capacitor se carga muy lentamente y el SCR dispara casi al final del semiciclo $\simeq 180^o$.
\midTitle{blue}{Notas del documentador:}{SCR}
\sangria{} Debido a la disponibilidad de las fuentes de $600V$ en el laboratorio tuvimos que hacer el trabajo junto a otros grupos.
